...
namespace NS_Planner
{
  ...
  void SimpleTrajectoryGenerator::initialise(
      ...
      const Eigen::Vector3f& goal, NS_Planner::LocalPlannerLimits* limits,
      ...
      bool discretize_by_time)
  {
    ...
    sample_params_.insert(sample_params_.end(), additional_samples.begin(),
                          additional_samples.end());
  }
  ...
  void SimpleTrajectoryGenerator::setParameters(double sim_time,
                                                double sim_granularity,
                                                double angular_sim_granularity,
                                                bool use_dwa, double sim_period)
  {
    sim_time_ = sim_time;
  ...
  bool SimpleTrajectoryGenerator::nextTrajectory(Trajectory &comp_traj)
  {
    ...
    {
      if(generateTrajectory(pos_, vel_, sample_params_[next_sample_index_],
                            comp_traj))
      {
        result = true;
      }
  ...
  bool SimpleTrajectoryGenerator::generateTrajectory(
      ...
      Eigen::Vector3f sample_target_vel, NS_Planner::Trajectory& traj)
  {
    ...
    if((limits_->min_trans_vel >= 0 && vmag + eps < limits_->min_trans_vel) && (limits_->min_rot_vel >= 0 && fabs(
        sample_target_vel[2]) + eps < limits_->min_rot_vel))
    {
      return false;
    }
    ...
    else
    {
      ...
      num_steps = ceil(
          std::max(sim_time_distance / sim_granularity_,
    ...
    if(continued_acceleration_)
    {
      ...
      loop_vel = computeNewVelocities(sample_target_vel, vel,
                                      limits_->getAccLimits(), dt);
      traj.xv_ = loop_vel[0];
      traj.yv_ = loop_vel[1];
    ...
    for(int i = 0; i < num_steps; ++i)
    {
      ...
      if(continued_acceleration_)
      {
        ...
        loop_vel = computeNewVelocities(sample_target_vel, loop_vel,
                                        limits_->getAccLimits(), dt);
  ...
  Eigen::Vector3f SimpleTrajectoryGenerator::computeNewPositions(
   ...
   */
  Eigen::Vector3f SimpleTrajectoryGenerator::computeNewVelocities(
      const Eigen::Vector3f& sample_target_vel, const Eigen::Vector3f& vel,
      Eigen::Vector3f acclimits, double dt)
  {
    ...
    for(int i = 0; i < 3; ++i)
    {
      ...
      {
        new_vel[i] = std::min(double(sample_target_vel[i]),
                              vel[i] + acclimits[i] * dt);
      }
      else
      {
        new_vel[i] = std::max(double(sample_target_vel[i]),
                              vel[i] - acclimits[i] * dt);
      }
    }
    return new_vel;
