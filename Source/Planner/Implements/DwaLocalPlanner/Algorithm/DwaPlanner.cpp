...
namespace NS_Planner
{
        ...
        alignment_costs_(planner_util->getCostmap())
  {
    ...
    generator_.setParameters(sim_time, sim_granularity, angular_sim_granularity,
                             dwa, sim_period_);
    ...
    obstacle_costs_.setParams(max_trans_vel, max_scaling_factor, scaling_speed);
  ...
  bool DWAPlanner::getCellCosts(int cx, int cy, float &path_cost,
                                ...
                                float &total_cost)
  {
    ...
    if(path_cost == path_costs_.obstacleCosts() || path_cost == path_costs_.unreachableCellCosts() || occ_cost >= NS_CostMap::INSCRIBED_INFLATED_OBSTACLE)
    {
      return false;
    }
   ...
   */
  bool DWAPlanner::checkTrajectory(Eigen::Vector3f pos, Eigen::Vector3f vel,
      ...
      const std::vector< NS_DataType::PoseStamped >& new_plan)
  {
    ...
    front_global_plan.back().pose.position.x = front_global_plan.back().pose.position.x + forward_point_distance_ * cos(
        angle_to_goal);
    front_global_plan.back().pose.position.y = front_global_plan.back().pose.position.y + forward_point_distance_ * sin(
        angle_to_goal);
   ...
   */
  NS_Planner::Trajectory DWAPlanner::findBestPath(
      NS_Transform::Stamped< NS_Transform::Pose > global_pose,
