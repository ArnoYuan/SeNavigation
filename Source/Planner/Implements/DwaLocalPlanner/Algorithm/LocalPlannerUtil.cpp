...
namespace NS_Planner
{
  ...
  void
  LocalPlannerUtil::initialize (NS_CostMap::Costmap2D* costmap)
  {
    if (!initialized_)
    {
      costmap_ = costmap;
  ...
  void
  LocalPlannerUtil::reconfigureCB (LocalPlannerLimits &config,
  ...
  LocalPlannerLimits
  LocalPlannerUtil::getCurrentLimits ()
  {
    boost::mutex::scoped_lock l (limits_configuration_mutex_);
    return limits_;
  }
