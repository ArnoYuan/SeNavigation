...
namespace NS_Planner
{
  ...
  MapGridCostFunction::MapGridCostFunction (NS_CostMap::Costmap2D* costmap,
      ...
      : costmap_ (costmap),
          ...
          aggregationType_ (aggregationType), xshift_ (xshift),
          yshift_ (yshift), is_local_goal_function_ (is_local_goal_function),
          stop_on_failure_ (true)
  {
  }
  
  void
  MapGridCostFunction::setTargetPoses (
      std::vector<NS_DataType::PoseStamped> target_poses)
  {
  ...
  double
  MapGridCostFunction::getCellCosts (unsigned int px, unsigned int py)
  {
