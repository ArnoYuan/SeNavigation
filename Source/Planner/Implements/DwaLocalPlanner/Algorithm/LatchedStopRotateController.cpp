...
namespace NS_Planner
{
      ...
      NS_Transform::Stamped< NS_Transform::Pose > global_pose)
  {
    ...
    if((latch_xy_goal_tolerance_ && xy_tolerance_latch_) || NS_Planner::getGoalPositionDistance(
        global_pose, goal_x, goal_y) <= xy_goal_tolerance)
    {
  ...
  bool LatchedStopRotateController::isGoalReached(
      LocalPlannerUtil* planner_util, OdometryHelper& odom_helper,
      NS_Transform::Stamped< NS_Transform::Pose > global_pose)
  {
    double xy_goal_tolerance = planner_util->getCurrentLimits().xy_goal_tolerance;
        ...
        global_pose, goal_x, goal_y) <= xy_goal_tolerance)
    {
      ...
      double angle = NS_Planner::getGoalOrientationAngleDifference(global_pose,
                                                                   goal_th);
  ...
  bool LatchedStopRotateController::stopWithAccLimits(
      ...
      double sim_period,
      boost::function<
          bool(Eigen::Vector3f pos, Eigen::Vector3f vel,
               Eigen::Vector3f vel_samples) > obstacle_check)
  {
    ...
    printf("Stopping cmd in collision\n");
               ...
               Eigen::Vector3f vel_samples) > obstacle_check)
  {
    ...
    double v_theta_samp = std::min(
        limits.max_rot_vel, std::max(limits.min_rot_vel, fabs(ang_diff)));

    //take the acceleration limits of the robot into account
    double max_acc_vel = fabs(vel_yaw) + acc_lim[2] * sim_period;
    double min_acc_vel = fabs(vel_yaw) - acc_lim[2] * sim_period;

    v_theta_samp = std::min(std::max(fabs(v_theta_samp), min_acc_vel),
                            max_acc_vel);
    ...
    v_theta_samp = std::min(limits.max_rot_vel,
                            std::max(limits.min_rot_vel, v_theta_samp));
    ...
    printf("Rotation cmd in collision\n");
    cmd_vel.angular.z = 0.0;
    return false;
               ...
               Eigen::Vector3f vel_samples) > obstacle_check)
  {
    ...
    if(!planner_util->getGoal(goal_pose))
    {
      printf("Could not get goal pose\n");
      return false;
    }
    ...
    if(latch_xy_goal_tolerance_ && !xy_tolerance_latch_)
    {
      printf("Goal position reached, stopping and turning in place\n");
      xy_tolerance_latch_ = true;
    }
    //check to see if the goal orientation has been reached
    double goal_th = NS_Transform::getYaw(goal_pose.getRotation());
