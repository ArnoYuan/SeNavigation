...
namespace NS_Planner
{
      ...
      NS_Transform::Stamped<NS_Transform::Pose> global_pose)
  {
    double xy_goal_tolerance =
        planner_util->getCurrentLimits ().xy_goal_tolerance;
    ...
    if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_)
        || NS_Planner::getGoalPositionDistance (global_pose, goal_x, goal_y)
            <= xy_goal_tolerance)
    {
      ...
      NS_Transform::Stamped<NS_Transform::Pose> global_pose)
  {
    ...
    double trans_stopped_vel =
        planner_util->getCurrentLimits ().trans_stopped_vel;
  ...
  bool
  LatchedStopRotateController::stopWithAccLimits (
      ...
      (Eigen::Vector3f pos, Eigen::Vector3f vel, Eigen::Vector3f vel_samples)> obstacle_check)
  {
    ...
    double vth = sign (vel_yaw)
        * std::max (0.0, (fabs (vel_yaw) - acc_lim[2] * sim_period));
      ...
      (Eigen::Vector3f pos, Eigen::Vector3f vel, Eigen::Vector3f vel_samples)> obstacle_check)
  {
    ...
    else
    {
      ...
      if (!rotating_to_goal_
          && !NS_Planner::stopped (base_odom, limits.rot_stopped_vel,
