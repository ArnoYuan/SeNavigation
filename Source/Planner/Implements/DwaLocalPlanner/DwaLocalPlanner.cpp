...
namespace NS_Planner
{
  ...
  DwaLocalPlanner::DwaLocalPlanner ()
      : initialized_ (false), odom_helper_ (), setup_ (false)
  {
  ...
  DwaLocalPlanner::onInitialize ()
  {
    if (!isInitialized ())
    {
      ...
      planner_util_.initialize (costmap2d);
      ...
      oscillation_reset_dist = parameter.getParameter ("oscillation_reset_dist",
                                                       0.05f);
      oscillation_reset_angle = parameter.getParameter (
          "oscillation_reset_angle", 0.2f);
      forward_point_dist = parameter.getParameter ("forward_point_dist",
                                                   0.325f);
      ...
      dp_ = boost::shared_ptr<DWAPlanner> (
          new DWAPlanner (&planner_util_, sum_scores, cheat_factor, sim_time,
                          sim_granularity, angular_sim_granularity, pdist_scale,
                          gdist_scale, occdist_scale, stop_time_buffer,
                          oscillation_reset_dist, oscillation_reset_angle,
                          forward_point_dist, max_trans_vel, scaling_speed,
                          max_scaling_factor, vx_samples, vy_samples,
                          vth_samples, dwa, sim_period));
      ...
      latchedStopRotateController_ = new LatchedStopRotateController (
  ...
  DwaLocalPlanner::isGoalReached ()
  {
    ...
    if (latchedStopRotateController_->isGoalReached (&planner_util_,
                                                     odom_helper_,
                                                     current_pose_))
    {
      printf ("Goal reached\n");
  ...
  bool
  DwaLocalPlanner::dwaComputeVelocityCommands (
      ...
      NS_DataType::Twist& cmd_vel)
  {
    ...
    NS_Planner::Trajectory path = dp_->findBestPath (
        global_pose, robot_vel, drive_cmds, costmap->getRobotFootprint ());
  ...
  bool
  DwaLocalPlanner::computeVelocityCommands (NS_DataType::Twist& cmd_vel)
  {
    ...
    {
      printf ("Received an empty transformed plan.\n");
      return false;
    }
    ...
    else
    {
      ...
      if (!isOk)
      {
        printf ("DWA planner failed to produce path.\n");
      }
      return isOk;
    }
  }
