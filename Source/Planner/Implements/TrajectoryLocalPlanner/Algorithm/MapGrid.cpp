...
namespace NS_Planner
{
  ...
  void
  MapGrid::sizeCheck (unsigned int size_x, unsigned int size_y)
  {
  ...
  inline bool
  MapGrid::updatePathCell (MapCell* current_cell, MapCell* check_cell,
      ...
      std::vector<NS_DataType::PoseStamped>& global_plan_out, double resolution)
  {
    ...
    for (unsigned int i = 1; i < global_plan_in.size (); ++i)
    {
      ...
      double sqdist = (loop_x - last_x) * (loop_x - last_x)
      ...
      const std::vector<NS_DataType::PoseStamped>& global_plan)
  {
    ...
    for (i = 0; i < adjusted_global_plan.size (); ++i)
    {
      ...
      if (costmap.worldToMap (g_x, g_y, map_x, map_y)
          && costmap.getCost (map_x, map_y) != NS_CostMap::NO_INFORMATION)
      {
  ...
  void
  MapGrid::computeTargetDistance (queue<MapCell*>& dist_queue,
                                  const NS_CostMap::Costmap2D& costmap)
  {
