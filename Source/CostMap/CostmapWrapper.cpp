...
namespace NS_CostMap
{
  ...
  CostmapWrapper::updateMap ()
  {
    ...
    if (getRobotPose (pose))
    {
      double x = pose.getOrigin ().x (), y = pose.getOrigin ().y (), yaw =
          NS_Transform::getYaw (pose.getRotation ());
  ...
  void
  CostmapWrapper::updateMapLoop (double frequency)
  {
  ...
  void
  CostmapWrapper::setPaddedRobotFootprint (
